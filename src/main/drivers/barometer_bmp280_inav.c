...
// BMP280, address 0x76
bool busWrite( uint8_t reg, uint8_t data);
bool busReadBuf( uint8_t reg, uint8_t *data, uint8_t length);

bool busRead(uint8_t reg ,uint8_t *data);
...
spl06_coeffs_t spl06_cal;    //bmp280_cal
...
{
    return busWrite(SPL06_MODE_AND_STATUS_REG, SPL06_MEAS_TEMPERATURE);
}
...
static bool spl06_read_temperature(void)
{
    ...
    bool ack = busReadBuf(SPL06_TEMPERATURE_START_REG, data, SPL06_TEMPERATURE_LEN);
...
{
    return busWrite( SPL06_MODE_AND_STATUS_REG, SPL06_MEAS_PRESSURE);
}
...
static bool spl06_read_pressure(void)
{
    ...
    bool ack = busReadBuf(SPL06_PRESSURE_START_REG, data, SPL06_PRESSURE_LEN);
...
bool spl06_calculate( int32_t * pressure, int32_t * temperature)
{
    ...
    if (pressure) {
        *pressure = (int32_t)(spl06_compensate_pressure(spl06_pressure_raw, spl06_temperature_raw));
    }

    if (temperature) {
        *temperature = (int32_t)(spl06_compensate_temperature(spl06_temperature_raw) * 100);
    }
...
static bool deviceDetect(void)
{
    ...
    for (int retry = 0; retry < DETECTION_MAX_RETRY_COUNT; retry++) {
        delay(100);
        bool ack = busRead(SPL06_CHIP_ID_REG, &chipId);
...
static bool read_calibration_coefficients(void) {
    uint8_t sstatus;
    if (!(busRead(SPL06_MODE_AND_STATUS_REG, &sstatus) && (sstatus & SPL06_MEAS_CFG_COEFFS_RDY)))
        return false;   // error reading status or coefficients not ready

    uint8_t caldata[SPL06_CALIB_COEFFS_LEN];

    if (!busReadBuf(SPL06_CALIB_COEFFS_START, (uint8_t *)&caldata, SPL06_CALIB_COEFFS_LEN)) {
        return false;
    }
...
static bool spl06_configure_measurements(void)
{
    ...
    if (!busWrite(SPL06_TEMPERATURE_CFG_REG, reg_value)) {
        return false;
    }

    reg_value = spl06_samples_to_cfg_reg_value(SPL06_PRESSURE_OVERSAMPLING);
    if (!busWrite(SPL06_PRESSURE_CFG_REG, reg_value)) {
        return false;
    }
    ...
    if (!busWrite(SPL06_INT_AND_FIFO_CFG_REG, reg_value)) {
        return false;
    }
...
bool busWrite( uint8_t reg, uint8_t data)
{
    return i2cWrite(BARO_I2C_INSTANCE, SPL06_I2C_ADDR, reg, data);
}

bool busReadBuf( uint8_t reg, uint8_t *data, uint8_t length)
{
    return i2cRead(BARO_I2C_INSTANCE, SPL06_I2C_ADDR, reg, length, &data);
}


bool busRead(uint8_t reg ,uint8_t *data){
    return i2cRead(BARO_I2C_INSTANCE, SPL06_I2C_ADDR, reg, 1, data);  
}
// yoosi end
